...
- **Machine `code` reference** (every refusal is `--json: {ok:false, …, code}`; exit 1 unless noted). *Input:* `invalid_workspace` (malformed id — same on `new`/`attach`/`drop`), `not_in_workspace` (no id and not on a `ws/<id>` branch), `invalid_task` (blank `-t`), `invalid_base` (bad `--base`), `no_commits` (empty/unborn repo — shared by `new`/`sync`/`land`/`validate`/`push`/`checkpoint save`), `unknown_branch` (`push -b <name>` where the branch doesn't exist — distinct from `no_commits`), `invalid_branch` (blank `--into`/`--as-branch`), `invalid_app` / `not_in_app_repo`, `missing_argument` / `unknown_command` / `no_command` (a `--json` call that fails CLI arg parsing — a missing required positional, an unknown/absent subcommand), `not_authenticated` (not logged in / session expired). *State:* `workspace_not_active` (finished — also carries the terminal `status` field), `foreign_workspace` / `branch_exists` / `dir_exists` / `worktree_setup_failed` (checkout claims & collisions), `no_default_branch` (no trunk yet). *Action-required (exit 2):* `diverged` (a peer published — merge first), `dirty_worktree`, `workspace_branch` (raw push/pull on a `ws/` branch), `merge_conflict`, `validation_failed` / `validation_mutated_tree`. *Transient/remote:* `fetch_failed`, `push_rejected`, `merge_failed` (a git merge error that is **not** a conflict — distinct from `merge_conflict`). Branch on `code`, never the prose.
