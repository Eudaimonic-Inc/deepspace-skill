...
- **Machine `code` reference** (every refusal is `--json: {ok:false, …, code}`; exit 1 unless noted). *Input:* `invalid_workspace` (malformed `ws_` id — same on `new`/`attach`/`sync`/`status`/`land`/`drop`), `not_in_workspace` (no id and not on a `ws/<id>` branch), `invalid_id` (malformed `chk_`/`hnd_` snapshot id — right prefix + bad body, or neither prefix; distinct from `wrong_kind`), `invalid_task` (blank `-t`), `invalid_base` (bad `--base`), `no_commits` (empty/unborn repo — shared by `new`/`sync`/`land`/`validate`/`push`/`checkpoint save`), `unknown_branch` (`push -b <name>` where the branch doesn't exist — distinct from `no_commits`), `invalid_branch` (blank `--into`/`--as-branch`/`--branch`), `invalid_app` / `not_in_app_repo` / `not_git_repo` (app dir exists but isn't a git repo) / `invalid_config` (unparseable `wrangler.toml`) / `git_too_old` / `unsupported_object_format` / `shallow_repo` (local git preconditions), `invalid_context_file` (unreadable `--context-file`), `invalid_release` (blank `rollback` id), `detached_head` (`push`/`pull` on a detached HEAD — pass `--branch`), `no_validate_command` (`validate`/`land --validate` with no script and no `--command`), `missing_argument` / `unknown_command` / `no_command` (a `--json` call that fails CLI arg parsing — a missing required positional, an unknown/absent subcommand), `not_authenticated` (not logged in / session expired). *State:* `workspace_not_active` (finished — also carries the terminal `status` field), `wrong_kind` (a real cross-kind snapshot id), `foreign_workspace` / `branch_exists` / `dir_exists` / `worktree_setup_failed` (checkout claims & collisions), `no_default_branch` (no trunk yet). *Action-required (exit 2):* `diverged` (a peer published — merge first), `dirty_worktree`, `workspace_branch` (raw push/pull on a `ws/` branch), `merge_conflict`, `trunk_moved` (trunk advanced mid-land — re-run to integrate), `validation_failed` / `validation_mutated_tree`. *Transient/remote:* `fetch_failed`, `fetch_incomplete` (a fetch that didn't materialize the expected ref — retry), `push_rejected` / `non_fast_forward` / `ref_conflict` / `rejected` (push/upload status), `rate_limited` (push HTTP 429), `merge_failed` (a git merge error that is **not** a conflict — distinct from `merge_conflict`). Every *Input* code (a blank `--app`, a malformed `-w`/id, a missing app/git repo) is emitted BEFORE any auth or network, so it's deterministic regardless of login state — you never get `not_authenticated` masking a bad argument. Branch on `code`, never the prose.
